"""M11 — Cresci-2017 full sweep: both engines, train/val/test protocol.

Runs the SybilSCAR and SybilHP engines over {G_T, G_B, fusion} × {neutral,
LR prior} on a fixed stratified train/val/test split, with all homophily
hyperparameters tuned on val and metrics reported on test. Covers both the
default ``social_spambots`` subsets and (with ``--include-optional``) the
``traditional_spambots`` + ``fake_followers`` subsets.

Usage::

    .venv/bin/python3 -m two_graph_fusion.scripts.run_cresci2017_tvt \\
        --cresci-root cresci_2017_datasets_full \\
        --output-dir two_graph_fusion/cache/cresci2017_tvt \\
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Cresci-2017 train/val/test sweep (SybilSCAR + SybilHP).",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--cresci-root", type=Path, default=Path("cresci_2017_datasets_full"))
    p.add_argument("--output-dir", type=Path, default=Path("two_graph_fusion/cache/cresci2017_tvt"))
    ...
    p.add_argument("--include-optional", action="store_true",
                   help="Also include traditional_spambots_1 and fake_followers.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    subset_tag = "with_optional" if args.include_optional else "default"
    log.info("=== Cresci-2017 TVT sweep (subsets=%s, seed=%d) ===", subset_tag, args.seed)

    # ------------------------------------------------------------------
    # 1. Labels, timestamps, features
    # ------------------------------------------------------------------
    label_map = load_labels(args.cresci_root, include_optional=args.include_optional)
    ts_map = load_timestamps(
        args.cresci_root, user_filter=set(label_map.keys()),
    ...
    # ------------------------------------------------------------------
    # 2. Graphs: G_B (behavioral k-NN), G_T (undirected) + directed trust edges
    # ------------------------------------------------------------------
    ...
    retweet_edges = load_retweet_edges(
        args.cresci_root, user_filter=qualifying_users,
        include_optional=args.include_optional,
    )
    reply_edges = load_reply_edges(
        args.cresci_root, user_filter=qualifying_users,
        include_optional=args.include_optional,
    )
    trust_edges_directed = retweet_edges + reply_edges
    g_t = nx.Graph()
    g_t.add_nodes_from(qualifying_users)
    g_t.add_edges_from(trust_edges_directed)

    gb_sum = graph_summary(g_b)
    gt_sum = graph_summary(g_t)
    log.info("G_B: nodes=%d edges=%d lcc=%.3f", gb_sum["n_nodes"], gb_sum["n_edges"], gb_sum["lcc_fraction"])
    log.info("G_T: nodes=%d edges=%d lcc=%.3f directed_edges=%d",
             gt_sum["n_nodes"], gt_sum["n_edges"], gt_sum["lcc_fraction"], len(trust_edges_directed))

    hom = {"g_t": _homophily_report(g_t, label_map_q), "g_b": _homophily_report(g_b, label_map_q)}

    # ------------------------------------------------------------------
    # 3. The full engine × graph × prior matrix
    # ------------------------------------------------------------------
    ...
    payload = {
        ...
        "gt": {"directed_edges": len(trust_edges_directed), **gt_sum},
        "homophily": hom,
    ...
    out_json = args.output_dir / f"results_{subset_tag}{rule_tag}{k_tag}{hyg_tag}_seed{args.seed}.json"
    with out_json.open("w") as fh:
        json.dump(payload, fh, indent=2, default=float)
    log.info("wrote %s", out_json)

    print(f"\n=== Cresci-2017 TVT sweep ({subset_tag}, seed={args.seed}) ===")
    ...
    for gname, lbl in [("g_t", "G_T (retweet+reply)"), ("g_b", "G_B")]:
        h = hom[gname]
        gate = "PASS" if h["homophily_lift"] >= 0.05 else "fail"
        print(f"  {lbl:<22} edges={h['n_edges_labeled']:>6,}  h={h['homophily']:.4f}  "
